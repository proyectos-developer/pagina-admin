...
import CardNotificacion from './card/notificacion.jsx'
...
export default function ListaNotificaciones ({proporcional}) {
    ...
    const dividir_nro_columnas = (data_notificaciones) => {
        ...
        let data = data_notificaciones.notificaciones.length
        let lista = []
        let cuenta = data / 4 < 1 ? 1 : data % 4 !== 0 ? (data / 4) + 1 : data / 4
        for (let count = 0; count < cuenta; count ++){
            lista.push ({num: `${count + 1}`})
        }
        setNotificaciones (data_notificaciones.notificaciones)
        setListaGridNotificaciones (lista)
        ...
        <div style={{width: '100%', height: 'auto', paddingLeft: open_menu_lateral ? 150 / proporcional : 250 / proporcional,
            ...
            </div>
            {
                lista_grid_notificaciones && lista_grid_notificaciones.length > 0 && view_notificacion === 'grid' ? (
                    lista_grid_notificaciones.map ((notificacion, numuni) => {
                        return (
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                {
                                    notificaciones [(4 * numuni)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardNotificacion notificacion={notificaciones[(4 * numuni)]} key={(4 * numuni)} index={(4 * numuni)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    notificaciones [((4 * numuni) + 1)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardNotificacion notificacion={notificaciones[((4 * numuni) + 1)]} key={((4 * numuni) + 1)} index={((4 * numuni) + 1)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    notificaciones [((4 * numuni) + 2)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardNotificacion notificacion={notificaciones[((4 * numuni) + 2)]} key={((4 * numuni) + 2)} index={((4 * numuni) + 2)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    notificaciones [((4 * numuni) + 3)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardNotificacion notificacion={notificaciones[((4 * numuni) + 3)]} key={((4 * numuni) + 3)} index={((4 * numuni) + 3)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                    ...
                    lista_notificaciones && lista_notificaciones.length > 0 && view_notificacion === 'lista' ? (
                        lista_notificaciones.map ((notificacion, numuni) => {
                            return (
                                <CardNotificacion notificacion={notificacion} key={numuni} index={numuni} proporcional={proporcional} view_notificacion={view_notificacion}/>
                            )
                        })
                ) : null
            }                   
