...
export default function ListaNotificacionesTablet ({proporcional}) {
                ...
                lista_grid_notificaciones && lista_grid_notificaciones.length > 0 && view_notificacion === 'grid' ? (
                    lista_grid_notificaciones.map ((notificacion, numuni) => {
                        return (
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                {
                                    notificaciones [(2 * numuni)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardNotificacionTablet notificacion={notificaciones[(2 * numuni)]} key={(2 * numuni)} index={(2 * numuni)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    notificaciones [((2 * numuni) + 1)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardNotificacionTablet notificacion={notificaciones[((2 * numuni) + 1)]} key={((2 * numuni) + 1)} index={((2 * numuni) + 1)} proporcional={proporcional} view_notificacion={view_notificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                    ...
                    lista_notificaciones && lista_notificaciones.length > 0 && view_notificacion === 'lista' ? (
                        lista_notificaciones.map ((notificacion, numuni) => {
                            return (
                                <CardNotificacionTablet notificacion={notificacion} key={numuni} index={numuni} proporcional={proporcional} view_notificacion={view_notificacion}/>
                            )
                        })
                ) : null
            }                   
