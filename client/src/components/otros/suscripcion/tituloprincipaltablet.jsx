...
export default function TituloPrincipalTablet ({proporcional, titulo, opcion}) {
    ...
    useEffect(() => {
        if (get_suscripciones && get_suscripciones.success === true && get_suscripciones.suscriptores){
            setSuscriptores(get_suscripciones.suscriptores.length)
            setTotalMostrando(get_suscripciones.suscriptores.length)
        }
    }, [get_suscripciones])
