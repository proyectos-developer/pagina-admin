...
export default function DetallesTareasProyecto({proporcional, proyecto}) {

    const dispatch = useDispatch()

    const [id_tarea, setIdTarea] = useState(proyecto.id)
    ...
    const obtener_datos_tarea = (tarea_id) => {
        dispatch(gestionproyectosdata(gestionproyectosConstants(tarea_id, 0, 0, 0, 0, 0, 0, 0, 0, {}, false).get_actividad_proyecto))
    }
                                    ...
                                    return (
                                        <h6 key={index} style={{fontSize: 14 / proporcional, lineHeight: `${30 / proporcional}px`, fontWeight: 500, marginBottom: 0,
                                            color: '#4a4a4a'}}>{index + 1}. <span style={{fontSize: 16 / proporcional, color: '#4a4a4a', fontWeight: 600,
                                                    cursor: 'pointer'}}
                                                onClick={() => obtener_datos_tarea(tarea.id)}>
                                                {tarea.tarea}
                                            </span>
                                        </h6>
                                    )
