...
export default function CardDocumentoTablet ({proporcional, index, documento}) {
    ...
    return (
        <div style={{width: '100%', height: 'auto', padding: 5 / proporcional}}
            onMouseOver={() => setOverCard(documento.id)} onMouseLeave={() => setOverCard('')}>
            <h6 className='rounded' key={index} 
                style={{fontSize: 16 / proporcional, lineHeight: `${24 / proporcional}px`, fontWeight: 500, marginBottom: 0,
                    color: over_card === documento.id ? 'white' : '#4a4a4a', background: over_card === documento.id ? '#007bff' : 'white'}}>{index + 1}. 
                    <span style={{fontSize: 16 / proporcional, fontWeight: 600, cursor: 'pointer'}}
                    onClick={() => obtener_datos_documento(documento.id)}> {documento.archivo} ({documento.versiones})
                </span>
            </h6>
        </div>
    )
