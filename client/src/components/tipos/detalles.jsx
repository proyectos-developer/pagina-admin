...
export default function DetallesTipoProyecto ({proporcional}) {
    ...
    const [enombre, setENombreNegocio] = useState(false)
    ...
    useEffect(() => {
        if (data_tipo_proyecto.nombre === undefined){
            dispatch(tipoproyectosdata(tipoproyectoConstants(location.pathname.split ('/')[3], {}, false).get_tipo_proyecto))
        }else{
    ...
    useEffect(() => {
        if (get_tipo_proyecto && get_tipo_proyecto.success === true && get_tipo_proyecto.tipo_proyecto){
            ...
            dispatch(tipoproyectosdata(tipoproyectoConstants(0, {}, false).get_tipo_proyecto))
        }
    }, [get_tipo_proyecto])

    useEffect(() => {
        if (update_tipo_proyecto && update_tipo_proyecto.success === true && update_tipo_proyecto.tipo_proyectos){
            dispatch(tipoproyectosdata(tipoproyectoConstants(0, {}, true).update_tipo_proyecto))
    ...
    const actualizar_datos_negocio = () => {
        if (nombre === ''){
            setENombreNegocio(nombre === '' ? true : false)
        }else{
            ...
            dispatch (tipoproyectosdata(tipoproyectoConstants(id_tipo, data_nuevo, false).update_tipo_proyecto))
        }
    }
