...
export default function CardFavoritoCell ({proporcional, index, favorito, view_favorito}) {
    ...
    useEffect(() => {
        axios.get(`${constantes().url_principal[0].url}/favorito/producto/clientes/${favorito.id_producto}/search/0/order_by/0/0/0/16`)
            .then ((res) => {
                setTotalFavoritos(res.data.total_favoritos)
            }).catch ((err) => {
    ...
    const ver_favorito = () => {
        ...
        navigate (`/panel/estadisticas/favoritos/producto/${favorito.producto.replace(' ', '-')}/${favorito.id_producto}`)
    }
                    ...
                    <div key={index} className={over_card ? 'rounded shadow-lg' : 'rounded shadow'} style={{width: '100%', height: '100%',
                        ...
                        <div style={{width: '100%', height: 'auto', padding: 20 / proporcional, cursor: 'pointer'}}>
                            <div className='d-flex justify-content-center' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                ...
                                <img className='rounded-circle' src={favorito.url_foto} style={{width: 148 / proporcional, height: 148 / proporcional}}/>
                                </div>
                            </div>
                            <div style={{width: '100%', height: 'auto'}}>
                                <h4 style={{fontSize: 16 / proporcional, lineHeight: `${40 / proporcional}px`, marginBottom: 16 / proporcional, 
                                    ...
                                    Producto: <span style={{color: '#007BFF', fontSize: 18 / proporcional}}>{favorito.producto}</span>
                                </h4>
                            </div>
                            <div style={{width: '100%', height: 'auto'}}>
                    ...
                    <div key={index} className='rounded' style={{width: '100%', height: 80 / proporcional, 
                        ...
                        <div style={{width: '100%', height: 80 / proporcional, padding: 10 / proporcional, cursor: 'pointer'}}
                            ...
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 60 / proporcional}}>
                                <div className='' style={{width: '90%', height: 60 / proporcional}}>
                                    <div style={{width: '100%', height: 'auto'}}>
