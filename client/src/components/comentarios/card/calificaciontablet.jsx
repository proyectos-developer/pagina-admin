...
export default function CardCalificacionTablet ({proporcional, index, calificacion}) {
    ...
    useEffect(() => {
        axios.get (`${constantes().url_principal[0].url}/producto/${calificacion.id_producto}`)
            .then ((res) => {
    ...
    const ver_calificaciones = () => {
        dispatch (set_data_calificaciones(calificacion))
        window.scrollTo(0, 0)
        //navigate (`/panel/calificaciones/producto/${producto.producto}/${producto.id}`)
    }
