...
export default function ListaCalificaciones ({proporcional}) {
    ...
    const dividir_nro_columnas = (data_calificaciones) => {
        ...
        let data = data_calificaciones.calificaciones.length
        let lista = []
        let cuenta = data / 4 < 1 ? 1 : data % 4 !== 0 ? (data / 4) + 1 : data / 4
        for (let count = 0; count < cuenta; count ++){
            lista.push ({num: `${count + 1}`})
        }
        setCalificaciones (data_calificaciones.calificaciones)
    ...
    const resetear_data = () => {
        ...
        setUnidad('Unidad')
        dispatch(calificacionesdata(calificacionesConstants(0, 0, 0, 0, 0, 0, 0, 100, {}, false).get_calificaciones_filter))
    }
            ...
            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 16 / proporcional}}>
                <div style={{width: '48%', height: 'auto'}}>
                    <h2 style={{fontSize: 28 / proporcional, lineHeight: `${30 / proporcional}px`, fontWeight: 500, marginBottom: 0,
                        color: '#4A4A4A'}}>Tus calificaciones
                ...
                lista_grid_calificaciones && lista_grid_calificaciones.length > 0 && view_calificacion === 'grid' ? (
                    lista_grid_calificaciones.map ((calificacion, numfav) => {
                        return (
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                {
                                    calificaciones [(4 * numfav)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardCalificacion calificacion={calificaciones[(4 * numfav)]} key={(4 * numfav)} index={(4 * numfav)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    calificaciones [((4 * numfav) + 1)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardCalificacion calificacion={calificaciones[((4 * numfav) + 1)]} key={((4 * numfav) + 1)} index={((4 * numfav) + 1)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    calificaciones [((4 * numfav) + 2)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardCalificacion calificacion={calificaciones[((4 * numfav) + 2)]} key={((4 * numfav) + 2)} index={((4 * numfav) + 2)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    calificaciones [((4 * numfav) + 3)] ? (
                                        <div style={{width: '24%', height: 'auto'}}>
                                            <CardCalificacion calificacion={calificaciones[((4 * numfav) + 3)]} key={((4 * numfav) + 3)} index={((4 * numfav) + 3)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '24%', height: 'auto'}}/>
                                    )
                    ...
                    lista_calificaciones && lista_calificaciones.length > 0 && view_calificacion === 'lista' ? (
                        lista_calificaciones.map ((calificacion, numfav) => {
                            return (
                                <CardCalificacion calificacion={calificacion} key={numfav} index={numfav} proporcional={proporcional} view_calificacion={view_calificacion}/>
                            )
                        })
                ) : null
            }            
