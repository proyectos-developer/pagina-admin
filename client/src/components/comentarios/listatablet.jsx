...
export default function ListaCalificacionesTablet ({proporcional}) {
    ...
    const [view_calificacion, setViewFavorito] = useState ('grid')
                ...
                lista_grid_calificaciones && lista_grid_calificaciones.length > 0 && view_calificacion === 'grid' ? (
                    lista_grid_calificaciones.map ((calificacion, numfav) => {
                        return (
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                {
                                    calificaciones [(2 * numfav)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardCalificacionTablet calificacion={calificaciones[(2 * numfav)]} key={(2 * numfav)} index={(2 * numfav)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    calificaciones [((2 * numfav) + 1)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardCalificacionTablet calificacion={calificaciones[((2 * numfav) + 1)]} key={((2 * numfav) + 1)} index={((2 * numfav) + 1)} proporcional={proporcional} view_calificacion={view_calificacion}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                    ...
                    lista_calificaciones && lista_calificaciones.length > 0 && view_calificacion === 'lista' ? (
                        lista_calificaciones.map ((calificacion, numfav) => {
                            return (
                                <CardCalificacionTablet calificacion={calificacion} key={numfav} index={numfav} proporcional={proporcional} view_calificacion={view_calificacion}/>
                            )
                        })
                ) : null
            }            
