...
export default function ModuloEmpresaPanelCell({proporcional}) {

    const navigate = useNavigate()
    const location = useLocation()

    const {data_servicio, data_departamento} = useSelector(({data_actions}) => data_actions)
                    ...
                    location.pathname.split('/')[6] !== undefined ? (
                        <div className='d-flex' style={{width: 'auto', height: 'auto'}}>
                            ...
                            <p style={{fontSize: 14 / proporcional, lineHeight: `${30 / proporcional}px`, color: 'rgb(89, 89, 89)',
                                    fontWeight: 500, fontFamily: 'Poppins, sans, serif', cursor: 'pointer',
                                marginRight: 10 / proporcional}}>
                                {data_servicio.servicio ? data_servicio.servicio.split(' ')[0] :
                                    data_departamento.departamento ? data_departamento.departamento.split(' ')[0] : ''} 
                            </p>
                        </div>
                    ) : null
                }
