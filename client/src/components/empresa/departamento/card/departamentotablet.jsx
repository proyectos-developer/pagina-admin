...
import { set_data_editable, set_data_departamento, set_confirmacion_eliminacion } from '../../../../redux/actions/data'
...
export default function CardDepartamentoTablet ({proporcional, index, departamento}) {
    ...
    useEffect(() => {
        if (confirmacion_eliminacion.confirmacion){
            ...
            dispatch(departamentosdata(departamentosConstants(confirmacion_eliminacion.id, 0, 0, 0, 0, 16, {}, false).delete_departamento))
        }
    ...
    const eliminar_departamento = () => {
        dispatch (set_confirmacion_eliminacion({show: true, confirmacion: false, mensaje: '¿Seguro que desea eliminar el departamento?', id: departamento.id}))
    }
                    ...
                    <div className='d-flex justify-content-between' style={{width: '70%', height: 30 / proporcional}}>
                        <div className='' style={{width: '48%', height: 30 / proporcional}}>
                            <p style={{fontSize: 16 / proporcional, lineHeight: `${30 / proporcional}px`, marginBottom: 0 / proporcional, 
                                ...
                                <span style={{color: '#007bff', fontSize: 16 / proporcional}}>{departamento.departamento}</span>
                            </p>
                        </div>
                        <div className='' style={{width: '48%', height: 30 / proporcional}}>
                            <p style={{fontSize: 16 / proporcional, lineHeight: `${30 / proporcional}px`, marginBottom: 0 / proporcional, 
                                color: 'rgb(89, 89, 89)', fontFamily: 'Merriweather', fontWeight: 600, textAlign: 'left'}}>
                                <span style={{color: '#007bff', fontSize: 16 / proporcional}}>{departamento.jefe}</span>
                            </p>
                        </div>
                    </div>
