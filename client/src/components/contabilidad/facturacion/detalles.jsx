...
import DatosCliente from './detalles/datoscliente.jsx'
import DatosFiscales from './detalles/datosfiscales.jsx'
import DatosProductosServicios from './detalles/datosproductosservicios.jsx'
import DatosFactura from './detalles/datosfactura.jsx'

export default function DetallesFactura ({proporcional}) {
    ...
    useEffect(() => {
        if (update_factura_cliente && update_factura_cliente.success === true && update_factura_cliente.factura){
            console.log ('update', update_factura_cliente)
    ...
    useEffect(() => {
        if (update_factura_fiscal && update_factura_fiscal.success === true && update_factura_fiscal.factura){
            console.log ('update', update_factura_fiscal)
    ...
    return (
        <div className='' style={{width: '100%', height: 'auto', paddingTop: 40 / proporcional, paddingBottom : 40 / proporcional}}>
            ...
            <div className='shadow' 
                style={{width: '90%', height: 'auto', background: 'white', padding: 100 / proporcional}}>
                    ...
                    <div style={{width: '100%', height: 'auto'}}>
                        {
                            datos_paso_factura === 'cliente' && factura.nombres ? ( 
                                <DatosCliente proporcional={proporcional} factura={factura}/>
                            ) : datos_paso_factura === 'fiscal' && factura.nombres ? (
                                <DatosFiscales proporcional={proporcional} factura={factura}/>
                            ) : datos_paso_factura === 'productos' && factura.nombres ? (
                                <DatosProductosServicios proporcional={proporcional} factura={factura}/>
                            ) : datos_paso_factura === 'factura' && factura.nombres ? (
                                <DatosFactura proporcional={proporcional} factura={factura}/>
                            ) : null
                        }
                    </div>
                </div>
