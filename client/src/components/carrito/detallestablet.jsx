...
export default function DetallesCompraTablet({proporcional}) {
  ...
  useEffect(() => {
    if (data_compras.fecha_compra === undefined){
        setShopId(location.pathname.split('/')[4])
        dispatch(comprasdata(comprasConstants(0, location.pathname.split('/')[4], 0, 0, 0, begin, amount, {}, false).get_productos_compra))
    }else{
        ...
        <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 16 / proporcional}}>
            <div style={{width: '90%', height: 'auto'}}>
                <h2 style={{fontSize: 28 / proporcional, lineHeight: `${30 / proporcional}px`, fontWeight: 500, marginBottom: 0,
                    color: '#4A4A4A'}}>Fecha de compra: <strong>{fecha_compra} </strong>
            ...
            lista_grid_productos && lista_grid_productos.length > 0 && view_compra === 'grid' ? (
                        ...
                        <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                            {
                                productos [(2 * numprod)] ? (
                                    <div style={{width: '48%', height: 'auto'}}>
                                        <CardProductoTablet producto={productos[(2 * numprod)]} key={(2 * numprod)} index={(2 * numprod)} proporcional={proporcional} view_compra={view_compra}/>
                                    </div>
                                ) : (
                                    <div style={{width: '48%', height: 'auto'}}/>
                                )
                            }
                            {
                                productos [((2 * numprod) + 1)] ? (
                                    <div style={{width: '48%', height: 'auto'}}>
                                        <CardProductoTablet producto={productos[((2 * numprod) + 1)]} key={((2 * numprod) + 1)} index={((2 * numprod) + 1)} proporcional={proporcional} view_compra={view_compra}/>
                                    </div>
                                ) : (
                                    <div style={{width: '48%', height: 'auto'}}/>
                                )
                ...
                lista_productos && lista_productos.length > 0 && view_compra === 'lista' ? (
                    lista_productos.map ((producto, numprod) => {
                        return (
                            <CardProductoTablet producto={producto} key={numprod} index={numprod} proporcional={proporcional} view_compra={view_compra}/>
                        )
                    })
            ) : null
        }  
