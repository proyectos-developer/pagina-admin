...
export default function TituloPrincipal ({proporcional, titulo, opcion}) {
    ...
    useEffect(() => {
        if (get_categorias && get_categorias.success === true && get_categorias.categorias){
            setNumeroCategorias(get_categorias.categorias.length)
            setTotalMostrando(get_categorias.categorias.length)
        }
    }, [get_categorias])
        ...
        <div style={{width: '100%', height: 60 / proporcional, paddingLeft: open_menu_lateral ? 150 / proporcional : 250 / proporcional,
            ...
            <h2 style={{fontSize: 28 / proporcional, lineHeight: `${30 / proporcional}px`, fontWeight: 500, marginBottom: 0,
                ...
                <span style={{fontSize: 16 / proporcional, color: 'rgb(89, 89, 89)'}}>
                    {opcion === 1 ? ` mostrando del 0 al ${total_mostrando} de ${numero_categorias}` : ''}
                </span>
            </h2>
        </div>
    )
