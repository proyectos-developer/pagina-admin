...
import CardCategoriaNoticiaTablet from './card/categoria_noticiatablet.jsx'
...
export default function ListaCategoriasNoticiasTablet ({proporcional}) {
                ...
                lista_grid_categorias_noticias && lista_grid_categorias_noticias.length > 0 && view_categoria_noticia === 'grid' ? (
                    lista_grid_categorias_noticias.map ((categoria_noticia, numcat) => {
                        return (
                            <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', marginBottom: 32 / proporcional}}>
                                {
                                    categorias_noticias [(2 * numcat)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardCategoriaNoticiaTablet categoria_noticia={categorias_noticias[(2 * numcat)]} key={(2 * numcat)} index={(2 * numcat)} proporcional={proporcional} view_categoria_noticia={view_categoria_noticia}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                                ...
                                {
                                    categorias_noticias [((2 * numcat) + 1)] ? (
                                        <div style={{width: '48%', height: 'auto'}}>
                                            <CardCategoriaNoticiaTablet categoria_noticia={categorias_noticias[((2 * numcat) + 1)]} key={((2 * numcat) + 1)} index={((2 * numcat) + 1)} proporcional={proporcional} view_categoria_noticia={view_categoria_noticia}/>
                                        </div>
                                    ) : (
                                        <div style={{width: '48%', height: 'auto'}}/>
                                    )
                    ...
                    lista_categorias_noticias && lista_categorias_noticias.length > 0 && view_categoria_noticia === 'lista' ? (
                        lista_categorias_noticias.map ((categoria_noticia, numcat) => {
                            return (
                                <CardCategoriaNoticiaTablet categoria_noticia={categoria_noticia} key={numcat} index={numcat} proporcional={proporcional} view_categoria_noticia={view_categoria_noticia}/>
                            )
                        })
                ) : null
            }             
