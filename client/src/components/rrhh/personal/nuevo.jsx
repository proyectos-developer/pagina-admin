...
export default function NuevoTrabajador ({proporcional}) {
    ...
    const {new_departamento, get_departamentos} = useSelector(({departamentos_data}) => departamentos_data)
    const {open_menu_lateral} = useSelector(({data_actions}) => data_actions)

    useEffect(() => {
        dispatch(departamentosdata(departamentosConstants(0, 0, 0, 0, 0, 100, {}, false).get_departamentos))
    }, [])

    useEffect(() => {
        if (get_departamentos && get_departamentos.success === true && get_departamentos.departamentos){
            setListaDepartamentos(get_departamentos.departamentos)
        }
    }, [get_departamentos])
                            ...
                            <div className='d-flex justify-content-between' 
                                ...
                                <div style={{width: '48%', height: 'auto'}}>
                                    ...
                                    <input
                                        ...
                                        className='form-control rounded'
                                        value={bonos}
                                        onChange={(event) => setBotonos(event.target.value)}
                                        style={{width: '100%', height: 50 / proporcional, fontSize: 16 / proporcional, color: 'rgb(89, 89, 89)',
                                                fontFamily: 'Poppins, sans-serif', border: '1px solid #007BFF',
                                                padding: 10 / proporcional}}
                                        placeholder='Bonos'/>
